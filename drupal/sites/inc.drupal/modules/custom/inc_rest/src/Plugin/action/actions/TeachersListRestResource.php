...
/**
 * class TeachersListRestResource
 *
 * Module de traitement pour une demande de la liste des enseignants.
 *
 * @Action (
 *   id = "action.plugin_id.teachers",
 *   label = @Translation("Module de traitement pour une demande de la liste des enseignants."),
 * )
 */
class TeachersListRestResource extends ActionResourceBase {
  ...
  public function apply(array $request, AccountProxyInterface $accountProxy): array
  {
    $students = Drupal::database()->query(
      "select
        ...
        "where (ufd.status = '1' and ufd.uid != '0' and ur.roles_target_id = 'enseignant')"
    )->fetchAll();
